...
import io.twdps.starter.boot.errorhandling.config.SecurityAllowConfig;
...
@ContextConfiguration(
    classes = {ErrorHandlerAdvice.class, ErrorHandlerConfig.class, SecurityAllowConfig.class})
public class ErrorHandlerAdviceContextTest {
  ...
  void whenResourceNotFound_thenReturns404() throws Exception {
    ...
    assertThat(error.getInstance().toString())
        .isEqualTo(String.format("https://starter.twdps.io/%s", traceparent));
  ...
  void whenTraceHeaderIsMissing_thenReturnsNoTrace() throws Exception {
    ...
    MockHttpServletResponse response =
        mockMvc
            .perform(post("/v1/example/test").contentType(MediaType.APPLICATION_JSON))
            .andReturn()
            .getResponse();
    ...
    assertThat(error.getType().toString()).isEqualTo("https://starter.twdps.io/request-validation");
    assertThat(error.getInstance().toString())
        .isEqualTo(String.format("https://starter.twdps.io/%s", "NOTRACE"));
    assertThat(error.getDetail()).contains("Required request body is missing");
    // assertThat(error.getReference()).isEqualTo("/v1/example/test");
  }
}
