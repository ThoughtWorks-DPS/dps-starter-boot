...
public class KafkaProducer<T extends IdentifiableMessage> {
  ...
  public KafkaProducer(
      KafkaTemplate<Integer, T> kafkaTemplate, KafkaProducerConfigProperties configProperties) {
  ...
  public ListenableFuture<SendResult<Integer, T>> sendMessage(String topic, final T message) {
    ...
    ListenableFuture<SendResult<Integer, T>> future =
        this.kafkaTemplate.send(topic, message.getMessageIdentifier(), message);
    return future;
  }
