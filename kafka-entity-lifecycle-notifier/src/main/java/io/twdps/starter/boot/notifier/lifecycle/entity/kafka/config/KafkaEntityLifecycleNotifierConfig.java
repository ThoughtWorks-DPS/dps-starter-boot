...
import io.twdps.starter.boot.notifier.lifecycle.entity.kafka.provider.KafkaEntityLifecycleNotifier;
...
@ConditionalOnProperty(
    prefix = "starter.boot.kafka-lifecycle-notifier",
    name = "enabled",
    havingValue = "true",
    matchIfMissing = true)
@AutoConfigureBefore(
...
public class KafkaEntityLifecycleNotifierConfig {
  ...
  public KafkaEntityLifecycleNotifierConfig(
    ...
    this.notifier =
        new KafkaEntityLifecycleNotifier(this.kafkaTemplate, this.configProperties, this.provider);
  }
