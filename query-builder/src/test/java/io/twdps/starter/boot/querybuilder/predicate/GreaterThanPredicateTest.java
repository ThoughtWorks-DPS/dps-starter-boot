...
class GreaterThanPredicateTest {
  ...
  private final GreaterThanPredicate<Object> greaterThanPredicate =
      new GreaterThanPredicate<>(new ObjectMapper(), Long.class);

  @Test
  void testToPredicateWhenSingleSearchCriteriaValue() {
    SearchCriteria searchCriteria =
        new SearchCriteria(false, "id", SearchOperation.GREATER_THAN_OR_EQUAL, "1");
    greaterThanPredicate.setSearchCriteria(searchCriteria);
    ...
    greaterThanPredicate.toPredicate(root, criteriaQuery, criteriaBuilder);

    verify(criteriaBuilder, times(1)).greaterThan(path, integerExpression);
  }
  ...
  void testToPredicateWhenNestedSearchCriteriaValue() {
    SearchCriteria searchCriteria =
        new SearchCriteria(false, "nested", SearchOperation.GREATER_THAN_OR_EQUAL, "id=1");
    greaterThanPredicate.setSearchCriteria(searchCriteria);
  ...
  void testToPredicateWhenOneToManySearchCriteriaValue() {
    SearchCriteria searchCriteria =
        new SearchCriteria(false, "list.nested", SearchOperation.GREATER_THAN_OR_EQUAL, "1");
    greaterThanPredicate.setSearchCriteria(searchCriteria);
  ...
  void testToPredicateWhenNestedOneToManySearchCriteriaValue() {
    SearchCriteria searchCriteria =
        new SearchCriteria(false, "list.nested", SearchOperation.GREATER_THAN_OR_EQUAL, "id=1");
    greaterThanPredicate.setSearchCriteria(searchCriteria);
