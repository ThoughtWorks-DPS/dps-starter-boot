...
public class KafkaProducerConsumerTest {
  ...
  @Autowired private CustomerEventKafkaConsumer eventConsumer;
  ...
  private String getRequestUrl() {
    return new StringBuilder(url).append(":").append(port).append(urlPath).toString();
  }
  ...
  public void testReceive() throws Exception {
    ...
    ResponseEntity<EventKafkaMetadata> response =
        this.restTemplate.postForEntity(getRequestUrl(), request, EventKafkaMetadata.class);
    ...
    assertThat(response.getBody().getCustomerId()).isEqualTo(customerEvent.getCustomerId());
    assertThat(response.getBody().getOffset()).isGreaterThan(-1);
  }
}
